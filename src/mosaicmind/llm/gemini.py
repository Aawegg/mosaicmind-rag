"""Gemini access through native Google integrations.

We deliberately avoid the OpenAI-compat shim because the native bindings
expose Gemini-specific features (system instructions, tools, caching,
native multimodal parts) that the compat layer flattens.

  - LangChain  : ChatGoogleGenerativeAI / ChatVertexAI  (LangGraph nodes)
  - LlamaIndex : GoogleGenAI / GoogleGenAIEmbedding      (query engines)
  - Native SDK : google.genai.Client                     (File API + audio/video)

Backend selection
-----------------
By default every client talks to the Gemini Developer API using
``GOOGLE_API_KEY``.  Set ``GOOGLE_GENAI_USE_VERTEXAI=true`` (plus
``GOOGLE_CLOUD_PROJECT`` and ``GOOGLE_CLOUD_LOCATION``) to route the same
Gemini models through GCP Vertex AI instead.  Vertex mode authenticates
with Application Default Credentials (ADC), so no API key is needed there.
"""
from __future__ import annotations

from functools import lru_cache

from llama_index.embeddings.google_genai import GoogleGenAIEmbedding
from llama_index.llms.google_genai import GoogleGenAI

from mosaicmind.config import get_settings


def _vertex_config() -> dict | None:
    """Return LlamaIndex-style vertex config when Vertex AI is enabled, else None."""
    s = get_settings()
    if not s.use_vertexai:
        return None
    if not s.gcp_project:
        raise ValueError(
            "GOOGLE_GENAI_USE_VERTEXAI is set but GOOGLE_CLOUD_PROJECT is empty. "
            "Set the project id (and optionally GOOGLE_CLOUD_LOCATION)."
        )
    return {"project": s.gcp_project, "location": s.gcp_location}


@lru_cache(maxsize=8)
def gemini_chat_model(model: str | None = None, temperature: float = 0.2):
    """LangChain chat model bound to a Gemini model.

    Uses ChatVertexAI on Vertex AI, otherwise ChatGoogleGenerativeAI on the
    Developer API. Both are lazily imported so neither is a hard dependency
    of the unused backend.
    """
    s = get_settings()
    if s.use_vertexai:
        cfg = _vertex_config()
        from langchain_google_vertexai import ChatVertexAI

        return ChatVertexAI(
            model=model or s.heavy_model,
            project=cfg["project"],
            location=cfg["location"],
            temperature=temperature,
            max_retries=2,
        )

    from langchain_google_genai import ChatGoogleGenerativeAI

    return ChatGoogleGenerativeAI(
        model=model or s.heavy_model,
        google_api_key=s.google_api_key,
        temperature=temperature,
        timeout=120,
        max_retries=2,
    )


@lru_cache(maxsize=2)
def gemini_embed_model() -> GoogleGenAIEmbedding:
    """LlamaIndex embedding model bound to gemini-embedding-001 (3072-dim)."""
    s = get_settings()
    vcfg = _vertex_config()
    if vcfg is not None:
        return GoogleGenAIEmbedding(
            model_name=s.text_embed_model,
            embed_batch_size=100,
            vertexai_config=vcfg,
        )
    return GoogleGenAIEmbedding(
        model_name=s.text_embed_model,
        api_key=s.google_api_key,
        embed_batch_size=100,
    )


@lru_cache(maxsize=4)
def gemini_llamaindex_llm(model: str | None = None, temperature: float = 0.2) -> GoogleGenAI:
    """LlamaIndex LLM bound to a Gemini model."""
    s = get_settings()
    vcfg = _vertex_config()
    if vcfg is not None:
        return GoogleGenAI(
            model=model or s.heavy_model,
            temperature=temperature,
            vertexai_config=vcfg,
        )
    return GoogleGenAI(
        model=model or s.heavy_model,
        api_key=s.google_api_key,
        temperature=temperature,
    )


@lru_cache(maxsize=1)
def gemini_native_client():
    """Native google-genai client for File API + native audio/video parts."""
    from google import genai

    s = get_settings()
    if s.use_vertexai:
        cfg = _vertex_config()
        return genai.Client(vertexai=True, project=cfg["project"], location=cfg["location"])
    return genai.Client(api_key=s.google_api_key)


# Kept for any callers that still want the raw OpenAI-compat client (optional).
def gemini_openai_client():
    """Optional: a raw OpenAI client pointed at Gemini's compat endpoint.

    Only used as an escape hatch for code paths that prefer the OpenAI SDK
    shape. Importing here lazily so `openai` is not a hard dependency.
    """
    from openai import OpenAI as OpenAIClient

    s = get_settings()
    return OpenAIClient(api_key=s.google_api_key, base_url=s.gemini_base_url)
